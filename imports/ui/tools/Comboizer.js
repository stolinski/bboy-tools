...
export class Comboizer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      randomMoves: [],
  ...
  getMoves = () => {
    ...
    const moves = this.props.moves.filter(
      move => filters.indexOf(move.type) > -1
    );
  ...
  render() {
    ...
    return (
      <div className="comboizer practice-tools types">
          ...
          {this.filters.map(filter => (
            <div className="comboizer-filter">
              <input
                ...
                onClick={() =>
                  this.setState({ [filter.type]: !this.state[filter.type] })
                }
              />
        ...
        {this.state.randomMoves.length > 0 && (
          <h3>{this.state.randomMoves[0].name}</h3>
        )}
        {this.state.randomMoves.length > 0 && (
          <h3>{this.state.randomMoves[1].name}</h3>
        )}
